...
export function dayRate(ratePerHour) {
  ...
  export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
    ...
    const totalRate = numDays*dailyRate;
    const totalMonths = Math.floor(numDays/22);
    const discountedProportion = 1 - discount;
  
    return Math.ceil(totalMonths*(dailyRate*22)*discountedProportion + (numDays%22)*dailyRate);  
  }